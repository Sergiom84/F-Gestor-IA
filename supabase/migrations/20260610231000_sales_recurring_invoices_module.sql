create table if not exists public.sales_recurring_invoices (
  ...
  constraint sales_recurring_invoices_org_matches_fiscal_entity
    check (organization_id = app_private.fiscal_entity_organization_id(fiscal_entity_id)),
  constraint sales_recurring_invoices_retention_rate_range
    check (retention_rate >= 0 and retention_rate <= 100),
  constraint sales_recurring_invoices_non_negative_suplido
    check (suplido_amount >= 0)
);

create table if not exists public.sales_recurring_invoice_lines (
  id uuid primary key default gen_random_uuid(),
  organization_id uuid not null references public.organizations(id) on delete cascade,
  sales_recurring_invoice_id uuid not null references public.sales_recurring_invoices(id) on delete cascade,
...
alter table public.sales_recurring_invoices enable row level security;
alter table public.sales_recurring_invoice_lines enable row level security;

create policy "org_member_recurring_invoices" on public.sales_recurring_invoices
  for all to authenticated
  using (app_private.is_org_member(organization_id))
  with check (app_private.is_org_member(organization_id));

create policy "org_member_recurring_invoice_lines" on public.sales_recurring_invoice_lines
  for all to authenticated
  using (app_private.is_org_member(organization_id))
  with check (app_private.is_org_member(organization_id));
...
alter table public.document_series
  ...
  check (doc_type in ('sales_invoice', 'sales_quote', 'sales_order', 'sales_delivery_note', 'sales_recurring_invoice'));
...
begin
  if target_doc_type not in ('sales_invoice', 'sales_quote', 'sales_order', 'sales_delivery_note', 'sales_recurring_invoice') then
  ...
  if clean_prefix = '' then
    clean_prefix := case
      ...
      when target_doc_type = 'sales_recurring_invoice' then 'REC'
      else 'FAC'
    end;
  end if;
